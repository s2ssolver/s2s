#!/usr/bin/python3
import argparse
import os
from subprocess import Popen, PIPE, TimeoutExpired
from timeit import default_timer as timer
import joblib


def parse():
    parser = argparse.ArgumentParser()
    parser.add_argument("dir", help="Directory containing the benchmarks")
    parser.add_argument("-b", help="Max bound", type=int, default=80)
    parser.add_argument(
        "-s", "--solver", help="Solver to use", default="iwoorpje")
    parser.add_argument(
        "-t", "--timeout", help="Timeout per instance in seconds", type=int, default=5)
    return parser.parse_args()


def run(file, bound, solver, timeout=1):
    print(file, end=": ")
    start = timer()
    proc = Popen(["./target/release/satstr", "-b", str(bound), "-s", solver, str(file)],
                 stdout=PIPE, stderr=PIPE)
    try:
        (stdout, stderr) = proc.communicate(timeout=timeout)

        time = timer() - start
        if stderr is not None and stderr.decode("utf-8").strip() != "":
            # print(stderr.decode("utf-8").strip())
            pass
        if stdout is not None:
            res = stdout.decode("utf-8").strip().splitlines()
            if len(res) == 0:
                print("❔ UNKNOWN: ", res)
                return (file, "unknown", time)
            if res[0] == "sat":
                print(f"✅ SAT ({time:.2f}s)")
                return (file, "sat", time)
            elif res[0] == "unsat":
                print(f"❌ UNSAT ({time:.2f}s)")
                return (file, "unsat", time)
            else:
                print("❔ UNKNOWN: ", res)
                return (file, "unknown", time)
    except TimeoutExpired:
        print("⏰ TIMEOUT")
        proc.kill()
        return (file, "timeout", timeout+1)


if __name__ == "__main__":
    args = parse()
    print("⚙️ Building...")
    Popen(["cargo", "build", "--release"],
          stdout=PIPE, stderr=PIPE).wait()
    results = joblib.Parallel(n_jobs=8)(joblib.delayed(run)(os.path.join(args.dir, file), args.b, args.solver, timeout=args.timeout)
                                        for file in os.listdir(args.dir))
    unsasts = [res[0] for res in results if res[1] == "unsat"]
    sats = [res[0] for res in results if res[1] == "sat"]
    t_total = sum([res[2] for res in results if res[1] != "timeout"])
    print(f"#️⃣  Total {len(results)} ({t_total:.2f}s)")
    print(f"❌ Unsats {len(unsasts)}")
    # print(" ".join(unsasts).strip(), end="")
    print(f"✅ Sats {len(sats)}")
    # print(" ".join(sats).strip(), end="")

# 80
